...
// 패킹 리스트 도착 상태 업데이트 API
router.put('/:id/arrival', auth, async (req, res) => {
  const connection = await pool.getConnection();
  
  try {
    const { id } = req.params;
    const { is_arrived, arrived_date } = req.body;
    const userId = req.user.id;
    
    console.log('📦 [packing-list-arrival] 도착 상태 업데이트 요청:', {
      id, is_arrived, arrived_date, userId
    });
    
    // 패킹 리스트 존재 여부 확인
    const [existingRows] = await connection.execute(
      'SELECT id, project_id FROM mj_packing_list WHERE id = ?',
      [id]
    );
    ...
    const updateQuery = `
      ...
      SET is_arrived = ?, arrived_date = ?, updated_at = NOW()
      WHERE id = ?
    `;
    
    await connection.execute(updateQuery, [is_arrived, arrived_date, id]);
    
    console.log('✅ [packing-list-arrival] 도착 상태 업데이트 완료:', {
      id, is_arrived, arrived_date
    });
    ...
    console.error('❌ [packing-list-arrival] 도착 상태 업데이트 오류:', error);
    res.status(500).json({ 
      success: false, 
      error: '도착 상태 업데이트 중 오류가 발생했습니다.' 
    });
  } finally {
    connection.release();
  }
...
// 패킹 리스트 도착 상태 일괄 업데이트 API
router.put('/batch-arrival', auth, async (req, res) => {
  const connection = await pool.getConnection();
  
  try {
    const { packing_list_ids, is_arrived, arrived_date } = req.body;
    const userId = req.user.id;
    
    console.log('📦 [packing-list-batch-arrival] 일괄 도착 상태 업데이트 요청:', {
      packing_list_ids, is_arrived, arrived_date, userId
    });
    ...
    // 패킹 리스트 존재 여부 확인
    const placeholders = packing_list_ids.map(() => '?').join(',');
    const [existingRows] = await connection.execute(
      `SELECT id FROM mj_packing_list WHERE id IN (${placeholders})`,
      packing_list_ids
    );
    
    if (existingRows.length !== packing_list_ids.length) {
      return res.status(400).json({ 
        success: false, 
        error: '일부 패킹 리스트를 찾을 수 없습니다.' 
      });
    }
    ...
    await connection.execute(updateQuery, [is_arrived, arrived_date, ...packing_list_ids]);
    
    console.log('✅ [packing-list-batch-arrival] 일괄 도착 상태 업데이트 완료:', {
      updated_count: packing_list_ids.length, is_arrived, arrived_date
    });
    
    res.json({
      success: true,
      message: `${packing_list_ids.length}개의 패킹 리스트 도착 상태가 성공적으로 업데이트되었습니다.`,
      data: {
        updated_count: packing_list_ids.length,
        is_arrived,
        arrived_date
      }
    });
    ...
    console.error('❌ [packing-list-batch-arrival] 일괄 도착 상태 업데이트 오류:', error);
    res.status(500).json({ 
      success: false, 
      error: '일괄 도착 상태 업데이트 중 오류가 발생했습니다.' 
    });
  } finally {
    connection.release();
  }
