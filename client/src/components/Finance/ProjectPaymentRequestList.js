...
import { Calendar, DollarSign, ChevronDown, ChevronRight, Clock, CheckCircle, Building, Package, Tag, Percent, Image, Printer } from 'lucide-react';
...
const ProjectPaymentRequestList = () => {
  ...
  // 프로젝트 지급완료 관련 상태
  const [completingPayments, setCompletingPayments] = useState(new Set()); // 프로젝트 지급완료 처리 중인 날짜들
  const [completedPayments, setCompletedPayments] = useState(new Set());   // 프로젝트 지급완료된 날짜들

  useEffect(() => {
    fetchProjectPaymentRequests();
  }, []);

  const fetchProjectPaymentRequests = async () => {
    try {
      setLoading(true);
      setError(null);
      
      const response = await fetch('/api/payment-request/project-payment-requests-by-date', {
        headers: {
      ...
      if (!response.ok) {
        throw new Error(`서버 오류 (${response.status}): 프로젝트 지급 요청 목록을 불러오는데 실패했습니다.`);
      }
      ...
      if (data.success) {
        ...
        throw new Error(data.message || '프로젝트 지급 요청 목록 조회에 실패했습니다.');
      }
    } catch (error) {
      console.error('프로젝트 지급 요청 목록 조회 오류:', error);
  ...
  const fetchDetailData = async (date) => {
    try {
      ...
      const response = await fetch(`/api/payment-request/project-payment-request-details/${date}`, {
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`
        }
      });
  ...
  const handlePrint = async (date) => {
    const request = projectRequests.find(r => r.date === date);
    if (!request) return;
  ...
  // 프로젝트 지급완료 처리 함수 (선금 + 잔금 통합)
  const handleCompleteProjectPayment = async (date) => {
    try {
      // 1. 해당 날짜의 선금/잔금 프로젝트 ID 수집
      const advanceProjects = detailData[date]?.advance || [];
      const balanceProjects = detailData[date]?.balance || [];
      
      const allProjectIds = [
        ...advanceProjects.map(p => p.project_id),
        ...balanceProjects.map(p => p.project_id)
      ];

      if (allProjectIds.length === 0) {
        alert('지급완료할 프로젝트가 없습니다.');
        return;
      }

      // 2. 확인 다이얼로그
      const confirmed = window.confirm(
        `${date} 날짜의 프로젝트 지급을 완료 처리하시겠습니까?\n` +
        `선금: ${advanceProjects.length}건, 잔금: ${balanceProjects.length}건\n\n` +
        `프로젝트 목록:\n` +
        `${advanceProjects.map(p => `- ${p.project_name} (선금)`).join('\n')}\n` +
        `${balanceProjects.map(p => `- ${p.project_name} (잔금)`).join('\n')}`
      );
      ...
      // 4. API 호출
      const response = await fetch('/api/payment-request/complete-project-payment', {
        method: 'POST',
        headers: {
      ...
      if (result.success) {
        ...
        alert(`프로젝트 지급완료 처리가 완료되었습니다.\n처리된 프로젝트: ${result.data.completedCount}개`);
        
        // 6. 데이터 새로고침
        await fetchProjectPaymentRequests();
        if (detailData[date]) {
          await fetchDetailData(date);
        }
      ...
      console.error('프로젝트 지급완료 처리 오류:', error);
    ...
    return (
      <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
        <div className="px-6 py-4 bg-gray-50 border-b border-gray-200">
          <h3 className="text-lg font-semibold text-gray-900">프로젝트 지급 요청 목록</h3>
        </div>
        <div className="p-6">
          <div className="text-center text-gray-500">
            <Clock className="w-8 h-8 mx-auto mb-2 animate-spin" />
            <p>프로젝트 지급 요청 목록을 불러오는 중...</p>
          </div>
        </div>
      </div>
    );
    ...
    return (
      <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
        <div className="px-6 py-4 bg-red-50 border-b border-red-200">
          <h3 className="text-lg font-semibold text-red-900">프로젝트 지급 요청 목록</h3>
        </div>
        <div className="p-6">
          <div className="text-center text-red-600">
            <p>오류가 발생했습니다: {error}</p>
            <button 
              onClick={fetchProjectPaymentRequests}
  ...
  return (
    <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
      <div className="px-6 py-4 bg-gray-50 border-b border-gray-200">
        <h3 className="text-lg font-semibold text-gray-900">프로젝트 지급 요청 목록</h3>
        <p className="text-sm text-gray-600 mt-1">날짜별로 그룹화된 프로젝트 지급 요청 현황 (선금/잔금)</p>
      </div>
      
      <div className="divide-y divide-gray-200">
        {projectRequests.map((request, index) => {
          const isExpanded = expandedDates.has(request.date);
          const hasAnyRequest = request.advance || request.balance;
            ...
            <div key={request.date} className="p-6">
              ...
              {isExpanded && (
                <div className="mt-4 space-y-6">
                  ...
                  {/* 통합 지급완료 버튼 */}
                  {(request.advance || request.balance) && (
                    <div className="flex justify-center pt-4">
                      <button
                        onClick={() => handleCompleteProjectPayment(request.date)}
                        disabled={completingPayments.has(request.date) || completedPayments.has(request.date)}
                        className={`inline-flex items-center px-6 py-3 text-sm font-medium rounded-lg transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 ${
