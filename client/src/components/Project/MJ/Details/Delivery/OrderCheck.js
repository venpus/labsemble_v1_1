...
const OrderCheck = ({ project, onDateChange, handleMultipleUpdates, isAdmin, isAdminLoading }) => {
  ...
  const handleDateChange = (field, value) => {
    if (field === 'actual_order_date') {
      const expectedFactoryShippingDate = calculateExpectedFactoryShippingDate(value);
      
      if (handleMultipleUpdates && typeof handleMultipleUpdates === 'function') {
        // 다중 업데이트 함수 사용
        const updateData = {
          actual_order_date: value,
          is_order_completed: value ? 1 : 0 // 날짜가 있으면 체크박스 활성화, 없으면 비활성화
        };
        ...
        // 기존 방식으로 개별 업데이트
        onDateChange('actual_order_date', value);
        onDateChange('is_order_completed', value ? 1 : 0);
        if (expectedFactoryShippingDate) {
          onDateChange('expected_factory_shipping_date', expectedFactoryShippingDate);
        }
      }
    ...
    <tr className="hover:bg-gray-50">
      ...
      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
        {isAdmin ? (
          <input
            type="date"
            value={project?.actual_order_date || ''}
            onChange={(e) => handleDateChange('actual_order_date', e.target.value)}
